...
| `loop.mjs`            | 재호출 드라이버 (1 호출 = 1 페이즈)                                           | `--init-plan[=path]` (정본 시드 — plan.json 의 source 인터뷰 문서 실존 필수). 구식 `--init "<s1>,<s2>"` 는 셀프테스트/CI 전용(`HARNESS_ALLOW_LABEL_INIT=1`/`HARNESS_SELFTEST=1` 필요). done 마감 전 최종 수용 검사(모든 step AC 커버리지) |
| `git-flow.mjs`        | `seed-main` / `start-step <nn> <slug>` / `merge-step <nn> <slug> [--vote-override] [--allow-empty] [--any-step]` | `skipGitFlow=true` 면 전 명령 no-op. merge-step 은 state.json 의 현재 step 만 병합(다른 step 은 `--any-step` 명시). main 전진 시 병합 결과 재게이트(실패 시 롤백). `--gate-ok`/`HARNESS_GATE_OK` 는 done-gate.mjs 부재 시 폴백(셀프테스트 전용) |
| `done-gate.mjs`       | 완료 게이트 (결정적 4종 + 평가 임계치)                                        | `--deterministic-only`, `--json`, `--score=N`, `--major-complaints=N`, `--skip-deterministic`. env 주입(`HARNESS_EVAL_SCORE/MAJOR`)은 `HARNESS_SELFTEST=1`/CI 에서만 유효(그 외 무시+경고) |
| `run-phase-session.mjs` | 격리 페이즈 세션 러너(세션 분리 3단계) — 에이전트 페이즈를 fresh `claude -p` 로 실행 + 세션 지문 발급 | `--phase=<decompose\|design\|implement\|debate\|vote>`(생략 시 state 의 현재 페이즈), `--timeout=ms`. 판정 세션은 쓰기 도구 없음. 교차 검증 강제는 `harness/config.json` `sessionIsolation: true` 옵트인 |
| `resume-watch.mjs` | 재개 루프 실행기 — state.json 이 stale(기본 30분)일 때만 인수해 loop/러너 재호출 | `--once`(1회 점검), `--interval-min=10`, `--stale-min=30`, `--max-burst=20`. 활성 세션(fresh state·잠금)에 양보, blocked 는 자동 재개 금지, 에이전트 페이즈는 격리 러너 가용 시에만 대행 |
