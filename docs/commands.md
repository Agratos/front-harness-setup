...
| `/clear-project` | **제자리에서** 이전 산출물·토큰·정체성 초기화                         | `reset-project.mjs`          | 예(미리보기→승인) |
| `/init-project`  | 시작 전 게이트: git/MCP + **git 원격·Notion 접근 확인** + `config.json` | `init-project.mjs`           | 아니오 |
| `/start-project` | Q&A → 계획(`planSteps`) 시드 → (git 시) main 시드                     | `loop.mjs --init`, `git-flow.mjs seed-main` | 아니오 |
...
```
새 프로젝트 시작 — 둘 중 하나
  (A) 다른 경로로:  /copy-project   ─┐  (복사 + 초기화를 한 번에)
  (B) 제자리:       /clear-project  ─┤
                                     ▼
                /init-project  →  /start-project  →  /run-cycle  ⟳ … (status=done)
                (연동 접근 확인)   (Q&A·계획·시드)    │
                                                      ├─ 내부: verify → /evaluate → debate/vote → merge(/git-flow)
                                                      └─ 진행 확인: /status (언제든)
```

- **부트스트랩(1회)**: `/copy-project` 또는 `/clear-project` 로 깨끗한 상태 → `/init-project` 로 연동 확인 → `/start-project` 로 계획 시드.
