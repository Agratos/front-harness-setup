...
## 시행 6 — `room-booking` (다른 고정 사양 · v3 2단계 검증)

하네스: main `d36880d` (tag `v3-stage2-phase-gate` — 페이즈 산출물 계약 적용 직후)
사양: `bench/room-booking/docs/spec/interview-2026-08-11.md` (회의실 예약 관리, 3 step / AC 11개)
목적: **계약이 실제 프로젝트에서 발화하는가** + 복잡한 도메인 규칙(시간 충돌)을 완주할 수 있는가

### 결과

| 지표 | 값 |
| --- | --- |
| 완주 | 3 step 전부 main 병합 → `status=done` |
| 재작업 / 에스컬레이션 / blocked | **0 / 0 / 0** |
| AC 달성 | **11/11** (실제 브라우저 조작으로 증명) |
| E2E | 시나리오 6개 · 단언 38건 전원 통과(이전 step 시나리오를 회귀로 누적) |
| 평가 | 96 → 100 → 100 (major 0) |
| 하네스 결함 발견 | **1건(minor, 아래 T6-1)** |
| 사람이 하네스를 고친 횟수 | 0 (시행 중 무수정 원칙 준수) |

### 계약이 발화한 지점 (실측)

| 페이즈 | 관측 |
| --- | --- |
| `decompose` (step01) | 기록 없이 호출 → **전진 차단** `산출물결함` + `failures: decompose=1/3` + 조치 명령 출력. 예전 하네스는 이 자리에서 아무 검사 없이 design 으로 전진했다 |
| `design` (step01·02·03) | 스펙·AC 계약이 **implement 전에** `AC 5/5 · 3/3 덮임` 을 확인. 스펙 부재로 verify 까지 갔다 되돌아오는 왕복이 발생하지 않았다 |
| `implement` (3회) | `코드 변경 확인(진입 시점 대비 diff 있음)` — 매 step 진입 시점 기준선이 새로 찍히는 것을 확인 |
| `debate` (3회) | 결정 기록 필수. 평가 결과를 근거로 pass 판정 후 merge |

### T6-1 · 🟡 minor — `계획에 없는 태그` 경고가 회귀 시나리오를 오탐한다

- **증상**: step02 의 design 계약 로그에 `· 계획에 없는 태그: AC-1, AC-2, AC-3, AC-4, AC-5`,
  step03 에는 `AC-1 ~ AC-8` 이 나열된다. 이 태그들은 **이전 step 의 AC** 이고, 스펙에 남겨 둔
  회귀 시나리오가 정상적으로 참조하는 것이다.
- **근본원인**: `lib/plan.mjs` 의 `checkAcCoverage(planStep, spec)` 가 **현재 step 의 AC 목록만** 알고
  있어서, 계획 전체에는 존재하는 id 도 "계획에 없는 태그"(오타 탐지 신호)로 분류한다.
- **영향**: 차단은 아니지만(경고 문구뿐) step 이 쌓일수록 목록이 길어져, **진짜 오타를 가린다.**
  오탐이 쌓이면 경고를 무시하게 되는 것이 이 하네스가 반복해서 고쳐 온 실패 유형이다.
- **Fix 후보**: `unknown` 판정을 **계획 전체의 AC id 집합** 기준으로 바꾸고, 다른 step 의 id 는
  `다른 step 의 AC(회귀)` 로 따로 표기한다.
- **상태**: 시행 종료 후 반영 예정(시행 중 무수정 원칙에 따라 기록만 함).

### 이번 시행이 확인한 것

1. **계약은 값을 냈다** — 실제 프로젝트 첫 호출에서 바로 발화했고, 조치 명령을 그대로 실행하니 전진했다.
   "무엇이 빠졌는지"를 사람이 추측하지 않았다.
2. **AC 를 design 에서 확정하게 만든 효과** — 스펙을 먼저 쓰게 되니 구현 중 "무엇을 만들어야 하는가"가
   흔들리지 않았고, 3 step 모두 재작업 0회로 통과했다.
3. **회귀 누적이 리팩터를 지켰다** — step02 에서 목록을 표시 전용(props)으로 바꾸고 widget 을 새로 넣었는데,
   step01 의 단언 15건이 자동으로 재검증되어 회귀 없음을 즉시 확인했다.
4. **이월 결함 처리가 계측으로 증명됐다** — step01 debate 에서 이월한 minor 2건을 step02 에서 고치니
