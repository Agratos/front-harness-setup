...
| 경로                | 무엇이 있나                                                                                                                                                                                                                       |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scripts/`          | 하니스 드라이버·게이트·평가·로깅 (Node `.mjs`). `loop.mjs`(드라이버), `git-flow.mjs`, `done-gate.mjs`, `eval-playwright.mjs`, `check-arch.js`, `preflight.mjs`, `demo.mjs`, `copy-project.mjs`(다른 경로로 복사+초기화), `reset-project.mjs`(제자리 초기화), `*.selftest.mjs`                                      |
...
| `.claude/commands/` | 슬래시 커맨드: `copy-project`(다른 경로로 복사+초기화), `clear-project`(제자리 초기화), `preflight`, `start-project`, `run-cycle`, `status`, `git-flow`, `evaluate`                                                                                                                                        |
