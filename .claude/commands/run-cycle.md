...
- **격리 세션 실행(세션 분리 3단계, 옵트인)**: `harness/config.json` 에 `sessionIsolation: true` 를 켠
  프로젝트에서는 에이전트 페이즈를 `node scripts/run-phase-session.mjs --phase=<페이즈>` 로 실행합니다 —
  fresh 헤드리스 세션이 페이즈를 수행하고 세션 지문이 기록에 찍힙니다. 이때 debate/vote 는
  **설계/구현과 다른 세션 지문**이 없으면 phase-gate 가 전진을 차단합니다(같은 손이 만들고 판정 금지).
