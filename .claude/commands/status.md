...
# 핵심 요약만
node -e "const s=JSON.parse(require('node:fs').readFileSync('harness/state.json','utf8')); console.log(`step ${s.currentStepIdx+1}/${s.planSteps.length} (${s.planSteps[s.currentStepIdx]??'-'}) | phase=${s.phase} phaseSeq=${s.phaseSeq} | status=${s.status} | rework=${s.reworkCount}`); console.log('scores:', JSON.stringify(s.scores));"
```
...
| `phase`                        | 현재(=다음 실행될) 페이즈. `decompose/design/implement/verify/evaluate/debate/merge`.      |
...
| `reworkCount`                  | 현재 step 재작업 횟수 (한도 5).                                                            |
